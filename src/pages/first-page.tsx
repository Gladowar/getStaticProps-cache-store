import { GetFirstPage } from '@/util/fetch-functions'
import { TextToObject } from '@/util/text-to-object'
import { FirstPageCacheSchema } from '@/validations/validate-cache'
import { GetServerSideProps, InferGetServerSidePropsType } from 'next'

const FirstPage = (props: InferGetServerSidePropsType<typeof getServerSideProps>) => {
  return (
    <>
      <p>
...
export const getServerSideProps = (async () => {
  try {
    console.time('cache')
    const data = await TextToObject('first-page', FirstPageCacheSchema)
    ...
    console.log({ error })
    console.time('fresh')
    const freshData = await GetFirstPage()
